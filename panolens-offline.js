...
var enableInlineVideo = function() {
	...
	function e(e, i, n, r) {
		function t(n) { d = i(t, r), e(n - (a || n)), a = n }
		var d, a;
		return { start: function() { d || t(0) }, stop: function() { n(d), d = null, a = 0 } }
	}

	function i(i) { return e(i, requestAnimationFrame, cancelAnimationFrame) }

	function n(e, i, n) {
		function r(r) { n && !n(e, i) || r.stopImmediatePropagation() }
		return e.addEventListener(i, r), r
	}

	function r(e, i, n, r) {
		function t() { return n[i] }

		function d(e) { n[i] = e } r && d(e[i]), Object.defineProperty(e, i, { get: t, set: d })
	}

	function t(e, i, n) { n.addEventListener(i, function() { return e.dispatchEvent(new Event(i)) }) }

	function d(e, i) { Promise.resolve().then(function() { e.dispatchEvent(new Event(i)) }) }

	function a(e) { var i = new Audio; return t(e, "play", i), t(e, "playing", i), t(e, "pause", i), i.crossOrigin = e.crossOrigin, i.src = e.src || e.currentSrc || "data:", i }

	function u(e, i, n) {
		(m || 0) + 200 < Date.now() && (e[h] = !0, m = Date.now()), n || (e.currentTime = i), k[++T % 3] = 100 * i | 0
	}

	function o(e) { return e.driver.currentTime >= e.video.duration }
	...
	function p(e, n) {
		var r = {};
		e[g] = r, r.paused = !0, r.hasAudio = n, r.video = e, r.updater = i(s.bind(r)), n ? r.driver = a(e) : (e.addEventListener("canplay", function() { e.paused || d(e, "playing") }), r.driver = { src: e.src || e.currentSrc || "data:", muted: !0, paused: !0, pause: function() { r.driver.paused = !0 }, play: function() { r.driver.paused = !1, o(r) && u(e, 0) }, get ended() { return o(r) } }), e.addEventListener("emptied", function() {
		...
		}, !1), e.addEventListener("webkitbeginfullscreen", function() { e.paused ? n && 0 === r.driver.buffered.length && r.driver.load() : (e.pause(), e[E]()) }), n && (e.addEventListener("webkitendfullscreen", function() { r.driver.currentTime = e.currentTime }), e.addEventListener("seeking", function() { k.indexOf(100 * e.currentTime | 0) < 0 && (r.driver.currentTime = e.currentTime) }))
	}

	function l(e) { var i = e[h]; return delete e[h], !e.webkitDisplayingFullscreen && !i }

	function f(e) {
		var i = e[g];
		e[E] = e.play, e[w] = e.pause, e.play = c, e.pause = v, r(e, "paused", i.driver), r(e, "muted", i.driver, !0), r(e, "playbackRate", i.driver, !0), r(e, "ended", i.driver), r(e, "loop", i.driver, !0), n(e, "seeking", function(e) { return !e.webkitDisplayingFullscreen }), n(e, "seeked", function(e) { return !e.webkitDisplayingFullscreen }), n(e, "timeupdate", l), n(e, "ended", l)
	}

	function y(e, i) {
		if (void 0 === i && (i = {}), !e[g]) {
			if (!i.everywhere) { if (!b) return; if (!(i.iPad || i.ipad ? /iPhone|iPod|iPad/ : /iPhone|iPod/).test(navigator.userAgent)) return } e.pause();
			var n = e.autoplay;
			e.autoplay = !1, p(e, !e.muted), f(e), e.classList.add("IIV"), e.muted && n && (e.play(), e.addEventListener("playing", function i() { e.autoplay = !0, e.removeEventListener("playing", i) })), /iPhone|iPod|iPad/.test(navigator.platform) || console.warn("iphone-inline-video is not guaranteed to work in emulated environments")
		}
	}
...
(function() {
	...
	PANOLENS.Utils.ImageLoader.load = function(url, onLoad, onProgress, onError) {
		...
		image.addEventListener('load', onImageLoaded, false);
		//console.log('=========' + cached + '=======' + image.crossOrigin + '======', url);
		image.src = url;
		/*
...
(function() {
	...
	PANOLENS.Widget.prototype.addControlBar = function() {
		...
		var scope = this,
			bar, styleTranslate, styleOpacity, gradientStyle;
...
(function() {
	...
	PANOLENS.Infospot = function(scale, imageSrc, animated) {
		...
		var scope = this,
			ratio, startScale, endScale, duration;
...
(function() {
	...
	PANOLENS.Viewer.prototype.outputInfospotPosition = function() {
		...
		if (intersects.length > 0) {
			...
			outputPosition = new THREE.Vector3(-(point.x - panoramaWorldPosition.x).toFixed(2),
	...
	PANOLENS.Viewer.prototype.update = function() {
		...
		this.scene.traverse(function(child) {
			if (child instanceof PANOLENS.Infospot && child.element) {
				if (this.checkSpriteInViewport(child)) {
					var vector = child.getWorldPosition();
					var vecCamera = this.camera.getWorldPosition();



					var vv = (vecCamera.distanceTo(vector) / ((83 * this.camera.fov) * this.camera.aspect)) * (this.camera.fov);
					//vv = vv * this.camera.aspect;
					vv = vv / this.camera.aspect;
					//				$(this.options.container).find('.infoPoint').hide();
					var vv = vecCamera.manhattanDistanceTo(vector);
					if ($(this.options.container).find('#pointCanvas' + child.FDATA.uid).length == 0) {
						//var d = $('<div class="infoPoint" style="position:absolute;z-index:6000;color:#ffffff" id="pointCanvas' + child.FDATA.uid + '">').appendTo($(this.options.container));
						//var a = $('<a href="#">').appendTo(d);

						//var i = $('<img src="' + child.FDATA.img + '">').appendTo(a);
					}
					//$(this.options.container).find('#pointCanvas' + child.FDATA.uid).css('left', vector.x);
					//$(this.options.container).find('#pointCanvas' + child.FDATA.uid).css('top', vector.y);
					//var ez = vv;
					//if (ez > 600) ez = 600;
					//ez = ez * this.camera.aspect;
					//ez = ((600 - ez)) / this.camera.aspect;
					var width = $(this.options.container).width();

					var ez = width - (width / (this.camera.fov * 10)) * 600;
					var ez = ((this.camera.fov * 10)) * (width * this.camera.aspect) / 1000;
					ez = (ez - width * this.camera.aspect) / this.camera.aspect;
					var ez = ((this.camera.fov * vv) / width);
					ez = (ez * (width / 600)) - width / this.camera.fov;
					ez = (ez / width) * (600 * this.camera.aspect);
					ez = ez / 1.8;
					if (ez < 300) {
						//	ez = 300 - (ez - 300)
					}

					if (this.hoverObject != child) {
						if (child.FDATA.type == 'popup' || child.FDATA.type == 'video' ||
							child.FDATA.type == 'audio' || child.FDATA.type == 'photo'
						) {
							child.scale.set(ez, ez, 1);

						}
						//if (child.FDATA.x == 3900)
						//	console.log(ez, child);


						//$(this.options.container).find('#pointCanvas' + child.FDATA.uid).html(ez);
						...
						/*
									...
									scope.scaleUpAnimation = new TWEEN.Tween(scope.scale)
										...
										.easing(TWEEN.Easing.Elastic.Out);
												...
												child.scaleFactor = 3.9;
												child.scaleUpAnimation.start();
						*/
					}
					//child.show();

					//$(this.options.container).find('#pointCanvas' + child.FDATA.uid).show();
					//console.log(this.options.container)scaleUpAnimation

					//console.log(child.FDATA, vector);
					//child.translateElement(vector.x, vector.y);

				}
				else {
					//$(this.options.container).find('#pointCanvas' + child.FDATA.uid).hide();
